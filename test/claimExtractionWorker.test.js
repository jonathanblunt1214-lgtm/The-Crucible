...
test('an exact source id selects only that queued source without advancing the backlog ahead of it',t=>{const target={...webSource(),id:'target'};const {root,queueFile}=fixture(t,target);const queue=JSON.parse(fs.readFileSync(queueFile,'utf8'));queue.links.unshift({...webSource(),id:'older'});fs.writeFileSync(queueFile,JSON.stringify(queue));const worker=new ClaimExtractionWorker({queueFile,projectId:'github:owner/repo',learningRoot:root,sourceId:'target',extractText:(source)=>`Source ${source.id} returns a bounded value.`});const outcomes=worker.run();assert.deepEqual(outcomes.map((item)=>item.sourceId),['target']);const after=JSON.parse(fs.readFileSync(queueFile,'utf8'));assert.equal(after.links.find((item)=>item.id==='older').state,'claim-extraction-forced-pending');assert.equal(after.links.find((item)=>item.id==='target').state,'claim-extraction-complete');});
