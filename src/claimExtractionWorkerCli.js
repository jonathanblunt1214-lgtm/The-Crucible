...
function run(argv = process.argv.slice(2), environment = process.env) {
  ...
  const worker = new ClaimExtractionWorker({ queueFile, projectId, learningRoot, maximumSources:Number(environment.CRUCIBLE_EXTRACTION_BATCH_SIZE || 20) }); const outcomes = worker.run();
