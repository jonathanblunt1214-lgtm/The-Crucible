...
function run(argv = process.argv.slice(2), environment = process.env) {
  ...
  const worker = new ClaimExtractionWorker({ queueFile, projectId, learningRoot, maximumSources:Number(environment.CRUCIBLE_EXTRACTION_BATCH_SIZE || 25), maximumDocuments:Number(environment.CRUCIBLE_EXTRACTION_MAX_DOCUMENTS || 9), pdfPagesPerBatch:Number(environment.CRUCIBLE_PDF_PAGES_PER_BATCH || 20) }); const outcomes = worker.run();
