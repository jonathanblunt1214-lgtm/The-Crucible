...
function run(argv = process.argv.slice(2), environment = process.env) {
  ...
  const worker = new ClaimExtractionWorker({ queueFile, projectId, learningRoot, maximumSources:Number(environment.CRUCIBLE_EXTRACTION_BATCH_SIZE || 25), maximumDocuments:Number(environment.CRUCIBLE_EXTRACTION_MAX_DOCUMENTS || 9), pdfPagesPerBatch:Number(environment.CRUCIBLE_PDF_PAGES_PER_BATCH || 20), sourceId:environment.CRUCIBLE_EXTRACTION_SOURCE_ID || null }); const outcomes = worker.run();
  return { recoveredFromForcedInterruption:worker.lastLockReclamation || null, processed:outcomes.length, completed:outcomes.filter((item) => item.state === 'claim-extraction-complete').length, continuing:outcomes.filter((item) => item.state === 'claim-extraction-forced-pending').length, blocked:outcomes.filter((item) => item.state === 'blocked').length, candidates:outcomes.reduce((sum, item) => sum + item.candidateIds.length, 0), sourceIds:outcomes.map((item) => item.sourceId) };
}
