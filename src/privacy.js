...
function findingsForText(value, githubIdentity) {
  ...
  for (const [type, rule] of RULES) {
    ...
    while ((match = rule.exec(content))) {
      ...
      findings.push({ type, line:content.slice(0, match.index).split(/\r?\n/).length });
    }
  }
  return findings;
}

function scrubText(value, githubIdentity) {
  ...
  content = content.replace(/\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/gi, (email) => isAllowedEmail(email, githubIdentity) ? email : '[REDACTED_EMAIL]');
  return content;
}
...
function auditPrivacy(root, config) {
  ...
  for (const file of files) {
    ...
    for (const finding of findingsForText(content, config.privacy.githubIdentity)) findings.push({ path:file, ...finding });
  }
...
function scrubPrivacy(root, config) {
  ...
  for (const file of trackedFiles(root)) {
    ...
    const after = scrubText(before, config.privacy.githubIdentity);
...
module.exports = { RULES, isAllowedEmail, findingsForText, scrubText, auditPrivacy, scrubPrivacy };
