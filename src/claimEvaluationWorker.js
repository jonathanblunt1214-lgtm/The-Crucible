...
class ControlledClaimEvaluationWorker {
  ...
  async process({ candidateId, corroboratingCandidateId, language, claimScope=null, ownerDeclaredAgreement=false }) {
    ...
    const reasoningPlan=this.reasoningProblemSolver.plan({candidate:primary.candidate,corroboratingCandidate:corroborating.candidate,comparison:decision,criticalReview,language,plannedAt:this.now()});this.reasoningLedger.record(reasoningPlan);
    ...
    const learner=new AutonomousScientificLearner({store:this.store,experimentExecutor:governedExperiment,independentVerifier:governedVerifier,now:this.now});const record=await learner.process(candidateId,reasoningPlan.hypothesis);
