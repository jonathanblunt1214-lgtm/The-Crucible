...
6. Let **The Crucible** report on development first. Do not require it on `main` until its workflow is present there and a representative pull request has produced the exact check.
...
10. Copy `templates/connect-workflow.yml` to `.github/workflows/connect-the-crucible.yml` on a repository whose literal default branch is `main`, replace `REPLACE_WITH_EXACT_COMMIT_SHA`, and run its `install` phase manually. It installs the permanent design brief, both AI ledgers, and both AI governance workflows on `main`, then creates `Development-branch` at that exact commit. It refuses to replace an existing development branch and does not activate branch rules in this phase.
