...
The operator machine, GitHub, and external AI services are execution hosts—not Crucible organs or learning sources. The boundary works like a selectively permeable membrane: authenticated, typed, project-bound traffic may cross governed gates, while unsafe or out-of-scope traffic is rejected or quarantined. GPU and ChatGPT egress require owner-signed permission plus an offline hard stop. See `governingDocuments/host-isolation-policy.md`.

Crucible's production organs communicate through one project-bound typed signal bus and durable heartbeat. Tests and diagnostics act as nerves, planning as the brain, reversible repair as the immune system, vetted learning intake as digestion, and circulation moves bounded work without becoming proof. Independent oversight can stop the organism; recovery requires oversight clearance and owner authorization. One health view reports each organ as healthy, degraded, inhibited, quarantined, or unavailable with its exact missing dependency.
...
Active knowledge is reviewed for age, contradictions, and changed sources. Old versions remain available for audit and rollback but leave active retrieval only after a distinct same-boundary replacement passes the full verification pipeline.

Governed learning by doing is available through `LearningExperienceRecorder` in `src/learningExperience.js`. It records content-addressed successful and failed task attempts as project-bound `experience-observation` candidates. These observations are deliberately non-promotable: Crucible must turn any proposed lesson into a separate controlled experiment and pass every ordinary scientific-learning gate before it can become verified knowledge.
...
The first owner-declared real-corpus scope is intentionally narrow: the two named GoalKicker and MDN assertions about `Array.prototype.map` may be evaluated only for ordinary dense numeric arrays on Node.js. Sparse arrays, proxies, subclasses, and host objects remain outside the claim. The declaration selects two existing hash-bound extracted candidates; it does not make either source true, run source text, bypass the critical reviewer, or authorize promotion. The controlled harness and a distinct verifier must still pass, and retrieved knowledge remains non-skipping regression context rather than proof for a later run.

Internet-assisted evidence discovery uses `SafeInformationRetriever` and `parseGoogleSearchResults` from `src/safeInformationRetrieval.js`. The retriever requires an exact owner-approved URL list, a persistent audit store, and an auditable kill-switch path. It performs credential-free HTTPS GET only and enforces DNS/IP, denylist, redirect, content-type, byte, timeout, rate, executable, and prompt-injection boundaries. Google parsing is passive discovery: only explicitly trusted domains, `.edu`, `.gov`, `.org`, or extreme-vetting `.science` results are emitted, while Wikipedia, Reddit, Facebook, `.onion`, Google navigation, denied, and unlisted domains are discarded. `.science` and recognized news agencies always require extreme vetting. `npm run learning:retrieve-sources` is the missing holding-to-extraction bridge: it retrieves only positively admitted queued URLs, stores sanitized content-addressed bytes under the same durable queue lock extraction uses, reuses an existing file for duplicate content, and emits only `Insufficient Evidence` work. Run `npm run test:safe-retrieval` and `npm run test:source-retrieval` for its adversarial security proof.

Automated Google discovery uses `AutomatedGoogleResearch` from `src/automatedGoogleResearch.js`. Approved topics become fixed safe-mode Google HTML queries limited to `.edu`, `.org`, and `.gov`; at most 50 due topics and ten results per topic are handled in one run. The client is credential-free, GET-only, rate/timeout/size/type bounded, private-address guarded, redirect-disabled, and controlled by `GOOGLE-RESEARCH-KILL` in the learning root. Its project-bound atomic store schedules each topic daily and deduplicates every discovered URL. New results enter the configured source queue only as `research-approved-pending-retrieval` and `Insufficient Evidence`; search pages and rankings are never evidence or proof. Set `CRUCIBLE_LEARNING_PROJECT_ID`, `CRUCIBLE_LEARNING_ROOT`, and `CRUCIBLE_SOURCE_QUEUE`, check `npm run learning:google-research-readiness`, then run `npm run learning:google-research -- "approved topic"`. Run `npm run test:automated-google-research` for its adversarial proof.

Daily Perplexity discovery is a second governed path for environments where Google rejects data-centre egress. It handles as many as 50 due topics per run, with each topic due once per day, and retains a project-bound holding queue across scheduled runs. `PERPLEXITY_API_KEY` is required; `PERPLEXITY_MODEL` is optional and the discovery command explicitly uses `sonar` when it is absent. Provider prose is discarded. Only structured citation URLs that independently pass the same HTTPS, denylist, and `.edu`/`.org`/`.gov` admission rules enter the queue, still labelled `Insufficient Evidence` and pending retrieval. A partially failed run emits the completed topic results and candidate IDs, then exits unsuccessfully. Use `npm run learning:perplexity-research-init`, `npm run learning:perplexity-research-readiness`, and `npm run learning:perplexity-research -- "approved topic"`; the read-only `perplexity-research.yml` workflow retains the queue and audit but grants no Oversight-state or promotion authority.

Exact owner-supplied PDFs, text files, and YAML research artifacts enter through `npm run learning:intake-owner-files -- <exact-file> [exact-file ...]`. Intake treats every byte as untrusted source data, never executes YAML or follows document instructions, validates readable PDF text before registration, and deduplicates across documents and retrieved links by SHA-256. Novel bytes are installed at a content-addressed path only after a complete copy is flushed and verified, then registered as `claim-extraction-forced-pending` and `Insufficient Evidence` while holding the same durable queue lock as extraction. Existing hashes are reported without copying or resetting their extraction history. Arbitrary malformed locks remain untouched and fail closed.

Logged learning URLs use `MonthlyKnowledgeRefreshStore` and `MonthlyKnowledgeRefresher` from `src/monthlyKnowledgeRefresh.js`. Registration is project-bound and content-addressed; each source is immediately due and then due again no later than 30 days after a successful check. A previously seen content hash produces no candidate work. Changed content is claim-deduplicated against every previously emitted or learned bounded claim for that URL and can emit only new `Insufficient Evidence` candidates. Refresh never writes verified knowledge: an update supersedes active knowledge only through the existing versioned scientific state machine after every proof gate passes. Run `npm run test:monthly-learning-refresh` for the refresh and anti-duplication proof.

Claim extraction records source and page-window content hashes, resumes only unfinished windows, skips unchanged completed windows, and restarts at page one only when the source content hash changes. Its external-PDF process boundary is injectable for deterministic proof, so tests do not depend on a shell alias masquerading as an executable. `npm run learning:compare -- <comparison.json>` performs a durable project-bound three-way routing comparison between two independently identified source claims and active verified knowledge. Agreement routes to controlled testing or records corroboration without relearning; same-scope disagreement and possible knowledge updates quarantine as `Crucible Issue`; scope or version differences remain bounded updates. Comparison, repetition, and corroboration explicitly satisfy no proof stage and cannot replace independent verification or promotion.

`ControlledClaimEvaluationWorker` connects candidate-pair selection and three-way routing to language-specific controlled and independent-verifier harness registries. It constructs a bounded falsifiable hypothesis, refuses identical source or executor identities, promotes only through `AutonomousScientificLearner`, and returns only the active version retrieved from durable knowledge. Harness implementations are trusted project code, never source-document text; unsupported languages or missing independent harnesses fail closed.
...
`CodeAssistiveSecurityOrganism` connects concrete detectors for JavaScript, TypeScript, Python, Java, C, C++, C#, PowerShell, Shell, HTML, CSS, React, and Django. A repository scan produces bounded findings plus whole-repository threat context for data flow, dependencies, authentication boundaries, secrets, injection paths, and repeated cross-file consequences. Repairs require an exact before/after boundary, base content hash, declared dependencies, a language-specific independent verifier, and rollback custody. Verified outcomes enter regression planning and learning as `Insufficient Evidence`; failed verification restores exact original content. Its health report marks detector, planner, executor, verifier, learning worker, and security gate as healthy, degraded, blocked, or inhibited and permits explicit recovery without losing state.
...
The pre-check and code-check decision path is governed in executable code by **Adapt, Persevere, Overcome**. Adapt inspects current repository content, installed tool behavior, validated configuration, and available upstream evidence in that order instead of enforcing a cached assumption. Persevere records each safe evidence or supported recovery attempt and, for a tool startup failure, retries once before treating the condition as unresolved. Overcome accepts a repair only after a passing re-check; known unsafe evidence still blocks, while genuinely ambiguous, semantic, or high-risk decisions are labeled `human code review required`. Unknown, stale, or unrecognized is never itself the reason for failure. The persisted latest report includes the governing decision and evidence-attempt outcomes. This mechanism never invents application requirements, rewrites business logic, or silently chooses between competing semantic interpretations.
...
Task routing is governed by the single canonical [`TASK-ROUTING.json`](TASK-ROUTING.json) on `development`. `npm run route:prewrite -- --prompt "<task>" --project "<project>" --path "<path>"` classifies wording, affected paths, and project context, then identifies the existing checkout for the stable repository ID and branch. Explicit destinations override automatic classification without bypassing branch access controls. Unknown or conflicting work stops for one focused question; multi-category work must be split into repository-scoped commits. Every ready decision is recorded in `AI-HANDOFF.json` and `DEVLOG.md`, while pre-push and development CI block mismatched destinations or paths. The router never creates or deletes branches, permits manual `ci-monitor` writes, makes Archive writable without exact owner approval, or bypasses the `release` to `main` promotion workflow. Other governed branches and repositories reference `development:TASK-ROUTING.json`; copies are forbidden.
...
10. Copy `templates/connect-workflow.yml` to `.github/workflows/connect-the-crucible.yml` on a new repository whose literal default branch is `main`, replace `REPLACE_WITH_EXACT_COMMIT_SHA`, and run its `install` phase manually. The bootstrap refuses any other default or dispatch branch, installs the permanent design brief, both AI ledgers, and both AI governance workflows on `main`, then creates the required development branch, named `Development-branch`, at that exact commit. The installed `AI-HANDOFF.json` declares a `governingDocuments` map naming every one of these files plus `templates/ai-conflict-resolution.md` and `templates/required-check-rollout.md`; the bootstrap installs those two governing-document templates as well (only if the repository does not already have its own file at that exact path, so nothing already there is overwritten) - a governing document a repository's own AI agents are expected to re-read must actually exist in that repository, not just be named. It refuses to replace an existing `Development-branch` and does not activate branch rules in this phase. A successfully initialized project therefore always has both a `main` branch and a development branch.
...
Multiple AI agents coordinate through the Shared AI handoff at the top of `DEVLOG.md` and the structured `AI-HANDOFF.json` development plan. Before beginning implementation, the starting AI records its identity, execution mode and purpose, objective, ordered steps, start/update times, status, completed work, verification, and remaining work so another AI can take over without private chat history; each new DEVLOG command-session heading also records `mode:regular/default` or `mode:work`, preserving mode in the chain of custody. Execution mode is independent from agent identity and workflow: regular/default mode serves focused routine work, while Work mode serves genuinely complex, long-running, multi-source, file-heavy, or deliverable-oriented work. Codex and Claude may each use either mode; changing agents must never silently substitute one mode for the other. When the owner says to work with, coordinate with, or continue work done with Codex, Claude, Perplexity, Gemini, or any later agent, the named agent must synchronize `development` and read both handoff surfaces before acting. Every agent must preserve concurrent work and keep both surfaces current in the same commit it pushes. Every conflict must be preserved and brought to the repository owner; no agent may silently choose, overwrite, delete, or revert one side.
...
This field list is generated directly from `WORKLOAD_FIELDS` in `src/config.js` by `npm run docs:sync` - the same array that validates and bounds these settings - so a field's default or bounds can never say one thing here and another in the code that enforces it. `npm run docs:check` fails Self-Test if it ever falls out of date.
...
- `maxOutputBytes`: Bytes of command output retained for failure messages, from 4096 through 10485760. Default: 1048576.
