...
1. Initialize project report.
2. Review dependency changes.
3. Check the installed design brief hasn't been deleted.
4. Verify the pinned Crucible commit is reachable and passed its own Self-Test.
5. Scan the checked-out Crucible engine code for malicious patterns.
6. Validate .thecrucible.json.
7. Enforce configuration and exception governance.
8. Lint GitHub Actions workflow permissions.
...
Multiple AI agents coordinate through the Shared AI handoff at the top of `DEVLOG.md`. When the owner says to work with, coordinate with, or continue work done with Claude or Codex, the named agent must synchronize `development` and read that handoff before acting. Every agent must preserve concurrent work and automatically keep the handoff's current plan, changes, verification, status, and remaining work accurate in the same commit it pushes. This makes the handoff visible to Codex, Claude, and human maintainers without depending on access to another agent's chat or requiring the owner to request an update.
