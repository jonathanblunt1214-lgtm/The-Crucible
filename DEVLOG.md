...
- **Agent:** Claude.
- **Current plan:** Confirmed `40db26b` (the `governingDocuments` cross-reference commit, naming `AGENTS.md`, `DEVLOG.md`, `AI-CONFLICTS.json`, `CLAUDE.md`, `README.md`, and the three policy templates AGENTS.md points to) is fully green on `development` across all six checks. The owner then asked for `governingDocuments` to be rechecked at the start of every session - same agent or a different one - any time more than 10 minutes have passed since the last recorded action in this repository, not just the first time a session starts.
- **Current development work:** Added a new "Recheck `governingDocuments` at the start of every session" rule to `AGENTS.md`, directly under the existing "Read `DEVLOG.md` first, always" rule, since they cover the same kind of gap (acting on stale context). Added a matching `sessionPolicy` object to `AI-HANDOFF.json`: `recheckGoverningDocuments` states the rule in the file itself, and `lastActionAt` is an ISO timestamp every future commit touching this file should refresh, so any agent - same or different - can compute how long it has actually been idle rather than guessing. This is deliberately a documentation/process rule, not something a GitHub Action can enforce: CI has no visibility into an AI agent's own session idle time, only into git activity, so the backstop is the same as every other AGENTS.md rule - it is binding on any agent reading it, and a session that skips it is violating a standing instruction, not exploiting a gap.
- **Files changed:** `AGENTS.md`, `AI-HANDOFF.json`, `DEVLOG.md`, `test/workflow.test.js`.
- **Verification:** Full local suite passes 207+/207+ (new assertions added for the AGENTS.md rule text and the `sessionPolicy` fields). Also ran `lint:workflows`, `docs:check`, `validate`, `audit:clutter`, `audit:privacy`, and `audit:security` - all pass.
- **Remaining work:** Push this commit to `development`, confirm all six checks are green, then fast-forward `release` to it so `main` picks up this rule too. `guard-ci-monitor-pr.yml` still has not been exercised live (no PR has been closed-without-merging since it was added), and per the standing rule in `AGENTS.md`, it should not be deliberately triggered just to test it - only a genuine close/merge event should exercise it. Separately, still pending: the repository owner needs to add `block` as a required status check on `main` (Settings -> Branches -> Branch protection rules) if not already done, for PR #11's lock to be GitHub-enforced - no tool available to any agent here can do that remotely. Still blocked on owner-supplied resources, not a code gap: the hosted multi-repository integration workflow and `connect-workflow.yml`'s `activate` phase both need a real target repository/token to actually be exercised.
