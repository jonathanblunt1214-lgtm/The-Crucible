...
- **Agent:** Claude.
- **Current plan:** Two owner-approved follow-ups: (1) give PR #9 the same GitHub-enforced never-merge backstop PR #7 had, and (2) make `templates/connect-workflow.yml`'s two-phase install/activate flow resumable after a partial failure - clarified with the owner first, since "never disconnect" as literally stated would have meant removing the documented design-brief-deletion severing check, which is deliberate anti-tampering, not a bug.
- **Current development work:** Extended `.github/workflows/block-pr-7.yml` (kept its original filename per the no-rename rule) to fail for either PR #7 or PR #9 via a `LOCKED_PR_NUMBERS` list, so the existing `block` check name carries over without needing the owner to re-register a new required-check name in `main`'s branch protection. Updated `AGENTS.md`'s absolute branch-policy rule to name PR #9 as the currently-protected instance, with PR #7 kept in the list purely as a historical safeguard (it can never reopen once merged) and a note that any future replacement should be added, never swapped in place of an old number. Separately, made `connect-workflow.yml`'s `install` job idempotent: if a prior run committed governance files to `main` but failed before creating `Development-branch` (network blip, concurrent push, runner loss), re-running install now recognizes an already-correct "nothing changed" diff as done rather than refusing it as a mismatch. Also tightened the `Development-branch` existence check to distinguish `git ls-remote --exit-code`'s exit 2 (branch genuinely doesn't exist - safe to create) from any other nonzero exit (a real connectivity/auth failure that must not be silently treated as "safe").
- **Files changed:** `.github/workflows/block-pr-7.yml`, `AGENTS.md`, `templates/connect-workflow.yml`, `test/workflow.test.js`, `AI-HANDOFF.json`, `DEVLOG.md`.
- **Verification:** Full local suite passes 204/204 (203 + 1 new, covering the connect-workflow resumability behavior). Also re-ran `lint:workflows`, `docs:check`, `validate`, `audit:clutter`, `audit:privacy`, and `audit:security` - all pass. The `activate` job's ruleset application was already idempotent (upsert-by-name) and needed no change.
- **Remaining work:** Push and confirm Self-Test, CodeQL, and AI handoff policy are green. The repository owner still needs to add `block` as a required status check on `main` (Settings -> Branches -> Branch protection rules) for PR #9's lock to actually GitHub-disable its merge button - no tool available to any agent here can do that remotely. Still blocked on owner-supplied resources, not a code gap: the hosted multi-repository integration workflow and `connect-workflow.yml`'s `activate` phase both need a real target repository/token to actually be exercised.
