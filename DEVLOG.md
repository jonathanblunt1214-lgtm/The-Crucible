...
- **Current development work:** Ran a full diagnostic on The Crucible's own test coverage. Audited every `src/*.js` module against `test/*.js`; most apparent gaps turned out to be real, solid coverage exercised indirectly through `hardening.test.js`/`engine.test.js`/`suiteSelection.test.js`/`code-check.test.js`, but four modules had genuinely no coverage at all: `authenticity.js`, `folderTopology.js`, `hostedMultiRepositoryIntegration.js`, `snapshot.js`. Added a dedicated `test/<module>.test.js` for each, following this repo's existing 1:1 naming convention. The owner then asked for a system to classify tests/audits by how often they need to run (every push vs. daily/weekly/monthly vs. reactively on a detected error) and to actually orchestrate execution by that classification rather than always running everything - confirmed scope via `AskUserQuestion` (both unit tests and audits, both manual and automated on-error). Built `src/testCadence.js`: an escalating-tier registry (`every-push` ⊂ `daily` ⊂ `weekly` ⊂ `monthly`) plus `ERROR_TRIGGERS` for read-only on-error diagnostics, with an orchestrator runnable as `npm run cadence:<tier>` / `npm run cadence:on-error -- <trigger>`. Critically, this is additive: `self-test.yml` still runs the full suite on every push unchanged; the new `scheduled-diagnostics.yml` workflow (report-only, cron-triggered) and an additive `if:failure()`/`continue-on-error:true` step appended to `self-test.yml` are the only new automation, and every on-error trigger is read-only-enforced by test, per the standing rule against invisible self-repair. Real (non-mocked) smoke-testing the orchestrator against this actual repository caught two real mistakes before they shipped: `audit:handoff` and `audit:required-check` both need a specific commit range/rollout context and fail by default, so both are deliberately excluded from every tier.
- **Files changed:** `AGENTS.md`, `AI-HANDOFF.json`, `DEVLOG.md`, `package.json`, `src/testCadence.js`, `.github/workflows/self-test.yml`, `.github/workflows/scheduled-diagnostics.yml` (new), `test/authenticity.test.js` (new), `test/folderTopology.test.js` (new), `test/hostedMultiRepositoryIntegration.test.js` (new), `test/snapshot.test.js` (new), `test/testCadence.test.js` (new), `test/workflow.test.js`.
...
### Session: pending (this commit) — 2026-08-27T21:39:09Z — Claude
