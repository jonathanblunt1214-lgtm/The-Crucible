...
- **Agent:** Claude.
- **Current plan:** Following a full CI diagnostic (all green: Self-Test/CodeQL/AI handoff policy on `development`, Self-Test/CodeQL on `main`, 206/206 local tests, all audits clean; `ci-monitor`'s decoupling from `development` verified structurally - not an ancestor of `main`, distinct commit SHA, identical tree content), the owner asked for two follow-ups: (1) run CodeQL, AI handoff policy, the local suite, and lint/docs/audits directly on `main`, and (2) something that permanently keeps PR #11 open no matter what.
- **Current development work:** For (1): checked each check - `codeql.yml` and `self-test.yml` (which runs the local suite plus `lint:workflows`/`docs:check`/every audit) already trigger on `push: branches: [main, development]`, so those three were already covered. Only `handoff-policy.yml` was missing `main` from its push trigger - it previously only ran on `push: branches: [development]` and `pull_request: branches: [main]` (which fires before a merge, not on the resulting push to `main` after a promotion merges). Added `main` to its push trigger so it also verifies the DEVLOG/AI-HANDOFF handoff on the actual post-merge commit landing on `main`, not just the pre-merge PR diff. For (2): added `.github/workflows/guard-ci-monitor-pr.yml`, triggered on `pull_request: types: [closed]`, which reopens PR #11 immediately if it is ever closed without being merged - the one thing `block-pr-7.yml`'s check cannot prevent, since a plain close needs no status check to pass. It only acts when the closed PR's head branch is `ci-monitor`, so no other pull request is ever touched. If PR #11 is ever actually merged instead, this workflow deliberately does nothing further - opening a replacement or rewriting `AGENTS.md` needs a human or an attended AI session, not invisible self-repair (matching this repository's standing rule from `internal-recovery.yml`'s own history). Documented both in `AGENTS.md`'s PR #11 bullet.
- **Files changed:** `.github/workflows/handoff-policy.yml`, `.github/workflows/guard-ci-monitor-pr.yml` (new), `AGENTS.md`, `test/workflow.test.js`, `AI-HANDOFF.json`, `DEVLOG.md`.
- **Verification:** Full local suite passes 207/207 (206 + 1 new, covering `guard-ci-monitor-pr.yml`'s content and the corresponding `AGENTS.md` documentation). Also ran `lint:workflows`, `docs:check`, `validate`, `audit:clutter`, `audit:privacy`, and `audit:security` - all pass.
- **Remaining work:** Push this commit to `development`, confirm Self-Test/CodeQL/AI handoff policy are green, then fast-forward `release` to it so the next `main` promotion exercises `handoff-policy.yml`'s new push:main trigger for real. `guard-ci-monitor-pr.yml` has not yet been exercised live (no PR has been closed-without-merging since it was added) - worth a manual sanity check if the owner wants to verify it before trusting it fully. Separately, still pending: the repository owner needs to add `block` as a required status check on `main` (Settings -> Branches -> Branch protection rules) if not already done, for PR #11's lock to be GitHub-enforced - no tool available to any agent here can do that remotely. Still blocked on owner-supplied resources, not a code gap: the hosted multi-repository integration workflow and `connect-workflow.yml`'s `activate` phase both need a real target repository/token to actually be exercised.
