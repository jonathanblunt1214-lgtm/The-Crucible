...
- **Agent:** Claude.
- **Current plan:** Fix an unintended consequence of the previous commit's promotion automation: PR #9 (the permanent, never-merge monitoring hook) was auto-merged and auto-closed by GitHub on its own, and the same thing will keep happening on every future promotion unless the monitoring hook's design changes.
- **Current development work:** Discovered that merging [PR #10](https://github.com/jonathanblunt1214-lgtm/The-Crucible/pull/10) (`release` -> `main`, `63a42ec`) made `development`'s history - including PR #9's exact head commit, since PR #9's head was the `development` branch ref directly - reachable from `main`. GitHub auto-detected this and closed+merged PR #9 on its own at 2026-08-27T20:00:28Z; nobody touched its merge button. This is structural, not a fluke: every future `release` promotion will do the same thing to whatever PR is currently serving as the monitoring hook, if that PR's head is `development` itself. Per the owner's explicit direction, redesigned the mechanism: created a dedicated `ci-monitor` branch whose own commits are distinct new commits copying `development`'s tree content - never `development`'s actual commit objects - so they can never become reachable from `main` through a `release` promotion. Added `.github/workflows/sync-ci-monitor.yml` (triggered on `development` pushes) to keep `ci-monitor` current this way. Opened [PR #11](https://github.com/jonathanblunt1214-lgtm/The-Crucible/pull/11) (`ci-monitor` -> `main`) as PR #9's replacement and subscribed to its activity. Extended `block-pr-7.yml`'s `LOCKED_PR_NUMBERS` to `"7 9 11"`. Updated `AGENTS.md` throughout: the absolute never-merge rule now names PR #11 and documents the incident and the fix, `ci-monitor` is added as an explicit branch-creation exception, and the "Automatic CI monitoring" / "CI on development" sections now reference PR #11 and `sync-ci-monitor.yml`.
- **Files changed:** `.github/workflows/sync-ci-monitor.yml` (new), `.github/workflows/block-pr-7.yml`, `AGENTS.md`, `test/workflow.test.js`, `AI-HANDOFF.json`, `DEVLOG.md`.
- **Verification:** Full local suite passes 206/206 (204 + 2 new, covering `sync-ci-monitor.yml`'s content and the corresponding `AGENTS.md` documentation). Also ran `lint:workflows`, `docs:check`, `validate`, `audit:clutter`, `audit:privacy`, and `audit:security` - all pass.
- **Remaining work:** Push this commit to `development`, confirm Self-Test/CodeQL/AI handoff policy are green, and confirm `sync-ci-monitor.yml` actually fires and commits a fresh snapshot onto `ci-monitor` (its first real exercise - `ci-monitor`'s current content was seeded manually from `development`'s prior tip, so this push is the first test of the automated sync path). Separately, still pending: the repository owner needs to add `block` as a required status check on `main` (Settings -> Branches -> Branch protection rules) if not already done, for PR #11's lock to be GitHub-enforced - no tool available to any agent here can do that remotely. Still blocked on owner-supplied resources, not a code gap: the hosted multi-repository integration workflow and `connect-workflow.yml`'s `activate` phase both need a real target repository/token to actually be exercised.
