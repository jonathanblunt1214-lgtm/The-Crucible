...
- **Agent:** Codex.
- **Current plan:** Add a reusable two-phase AI-link bootstrap for future repositories: install governance and create `Development-branch` first, then activate rules only after the exact checks are verified on `main` and a representative pull request.
- **Current development work:** Extended the repository-local AI link with separate `install` and `activate` phases. Installation is manually dispatched from literal `main`, installs exactly the five mandatory governance files, and creates `Development-branch` without replacing an existing branch. Activation fails closed without exact owner confirmation, a representative `main` pull request whose head has successful **AI conflict governance** and **AI handoff policy** checks, and a repository-scoped Administration token. Only then does it apply zero-bypass default-branch and AI branch-scope rulesets.
- **Files changed:** `templates/connect-workflow.yml`, `test/workflow.test.js`, `README.md`, `AI-HANDOFF.json`, `DEVLOG.md`.
- **Verification:** Full local suite passes 172/172. Workflow permissions lint, documentation consistency, configuration validation, AI conflict governance, clutter, privacy, security, and `git diff --check` pass.
- **Remaining work:** Push the verified commit only to `development` and confirm hosted Self-Test and CodeQL. The link is reusable per repository; a separate repository template or organization provisioning service must place it into future repositories automatically.
