...
- **Agent:** Claude.
- **Current plan:** Confirmed `47383c7` (adding `main` to `handoff-policy.yml`'s push trigger, and `guard-ci-monitor-pr.yml`) is fully green on `development` (Self-Test, CodeQL, AI handoff policy all success). Also updated the hourly fallback CI-monitoring routine's own stored prompt, which still referenced merged PR #9 as the live monitoring hook - it now names PR #11 and covers all three checks (Self-Test, CodeQL, AI handoff policy), not just the original two. Per the owner's explicit standing instruction: a future broad request like "test everything" or "test all systems" must never be read as license to deliberately close or merge PR #11 to exercise `block-pr-7.yml`'s lock or `guard-ci-monitor-pr.yml`'s reopen behavior live - that would mean violating the exact policy those systems enforce just to check that the enforcement works. Those two are only genuinely exercised if PR #11 is actually closed or merged by some other means; that real event is the test.
- **Current development work:** Documented the "never deliberately test PR #11's lock/guard by closing or merging it" rule in `AGENTS.md`'s PR #11 bullet, as a new sub-bullet directly after the `guard-ci-monitor-pr.yml`/`block-pr-7.yml` description. Added a matching assertion to the existing `guard-ci-monitor-pr.yml` test in `test/workflow.test.js`. Separately (not a code change): updated the hourly fallback trigger's stored prompt (`trig_01DK6joDQdxqGaoQVFCAHzKd`) to name PR #11 instead of the now-merged PR #9, and to check AI handoff policy alongside Self-Test/CodeQL.
- **Files changed:** `AGENTS.md`, `test/workflow.test.js`, `AI-HANDOFF.json`, `DEVLOG.md`.
- **Verification:** Full local suite passes 207/207 (206 + 1 updated assertion). Also ran `lint:workflows`, `docs:check`, `validate`, `audit:clutter`, `audit:privacy`, and `audit:security` - all pass.
- **Remaining work:** Push this commit to `development`, confirm Self-Test/CodeQL/AI handoff policy are green, then fast-forward `release` to it so the next `main` promotion carries this documentation update too. `guard-ci-monitor-pr.yml` still has not been exercised live (no PR has been closed-without-merging since it was added), and per the new rule above, it should not be deliberately triggered just to test it - only a genuine close/merge event should exercise it. Separately, still pending: the repository owner needs to add `block` as a required status check on `main` (Settings -> Branches -> Branch protection rules) if not already done, for PR #11's lock to be GitHub-enforced - no tool available to any agent here can do that remotely. Still blocked on owner-supplied resources, not a code gap: the hosted multi-repository integration workflow and `connect-workflow.yml`'s `activate` phase both need a real target repository/token to actually be exercised.
